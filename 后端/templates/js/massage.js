...
function showMessage1(txt){
    ...
    if(!oDiv){
        ...
        document.getElementById('tan').appendChild(oDiv);
    }
    oDiv.innerHTML = '<div class="active" style="width: 350px;position: absolute;height: 100px;left: 264px;top: 258px;font-size: 30px">'+'<span>'+txt+'</span>'+'</div>';
    oDiv.classList.remove('hide');//默认是显示
    this.timer =
        setTimeout(function(){
            oDiv.classList.add('hide');
            oDiv.hidden.valueOf("hidden")
            // var t = document.getElementById("right_bottom");
            // t.classList.remove('hide');//2s后隐藏
        },2000)
}
